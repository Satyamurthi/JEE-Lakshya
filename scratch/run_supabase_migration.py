...
print("Fetching all questions from SQLite DB for full Supabase push...")
cursor.execute("""
...
print(f"Loaded {len(db_questions)} questions from SQLite database.")
...
for row in db_questions:
    ...
    if type_upper == "MCQ":
        ...
        options_dict = {}
        correct_answer = db_options[0][0] if db_options else "10"
...
print("Initiating full batch upload to Supabase REST API...")
batch_size = 100
uploaded = 0
...
print(f"\nSupabase Full Sync complete! Total questions pushed: {uploaded}")
