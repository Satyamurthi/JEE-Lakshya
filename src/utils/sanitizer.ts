...
export const cleanQuestionText = (text: string): string => {
  ...
  // 10b. Pre-convert Unicode Greek letters and math operators to TeX BEFORE preprocessTeXMacros
  // (Critical: frac/vec patterns in preprocessTeXMacros only match [a-zA-Z], so Δ must become \Delta first)
  cleaned = cleaned
    .replace(/×/g, '\\times ')
    .replace(/÷/g, '\\div ')
    .replace(/≤/g, '\\leq ')
  ...
  // 11. Preprocess TeX macros (\\matrix, \\over, \\cr, brace fixes, bare-macro conversions etc.)
