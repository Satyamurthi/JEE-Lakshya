...
def generate_master_sql():
    ...
    sql_out = os.path.join(os.path.dirname(__file__), "master_neet_setup.sql")
    ...
    with open(sql_out, "w", encoding="utf-8") as f:
        f.write("-- ========================================================\n")
        f.write("-- 🛡️ STEP 1: FIX RLS POLICIES FOR QUESTIONS TABLE\n")
        f.write("-- ========================================================\n")
        f.write("alter table public.questions enable row level security;\n")
        f.write("drop policy if exists \"Admin update questions\" on questions;\n")
        f.write("create policy \"Admin update questions\" on questions for update using (public.is_admin(auth.uid()) OR auth.uid() is null);\n")
        f.write("drop policy if exists \"Admin delete questions\" on questions;\n")
        f.write("create policy \"Admin delete questions\" on questions for delete using (public.is_admin(auth.uid()) OR auth.uid() is null);\n\n")

        f.write("-- ========================================================\n")
        f.write("-- 🧹 STEP 2: CLEAR ALL OLD QUESTIONS\n")
        f.write("-- ========================================================\n")
        f.write("TRUNCATE TABLE public.questions;\n\n")

        f.write("-- ========================================================\n")
        f.write("-- 🚀 STEP 3: INSERT FRESH 8-YEAR NEET PYQS (56 QUESTIONS)\n")
        f.write("-- ========================================================\n")
    ...
    conn.close()
    print(f"[SUCCESS] Master SQL generated at {sql_out}")
