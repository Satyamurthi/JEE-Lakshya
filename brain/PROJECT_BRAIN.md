...
*   **Cloudflare Tunnel**: Exposing local API Port 80. For named/persistent Zero Trust Tunnels, configure the **Service Type** as `HTTP` and the **Service URL** as `http://localhost` (or `http://localhost:80`). Quick/Ephemeral tunnel URLs generated by `scripts/run_tunnel.ps1` are published automatically to `public/backend_url.txt` in the GitHub repository.
*   **Routing proxy**: Supabase integrations and environment credentials are completely unlinked. All frontend queries are parsed by the client proxy builder inside `src/supabase.ts` and routed directly to the dynamic backend URL (either from `VITE_API_URL` or relative `backend_url.txt`) which is resolved locally on the MariaDB databases via `local_db.php`.
*   **Super Admin Authorization**: Hardcoded email elevations are removed. Dynamic super admin authorization is verified directly against user profile roles in the databases, populated automatically upon initial catalog creation by `setup_db.php`.
*   **Production Environment**: Pushed `netlify.toml` which forces Netlify builds to use `VITE_USE_LOCAL_SERVER = true` and `VITE_API_URL = http://localhost/api`.
