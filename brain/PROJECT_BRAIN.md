...
*   **Exponential Retry Backoff for NVIDIA NIM**: Added a 3-attempt retry loop with exponential sleep backoff (3s for first retry, 6s for second retry) on `429 Too Many Requests` rate-limit errors in `callNvidiaAPI`. This shields the generation flows from failing when rate limits are temporarily exceeded.
