...
*   **Cloudflare Tunnel**: Cloudflare Tunnel is running 24/7 on the local PC via the `CloudflareTunnel` Windows Scheduled Task. It starts the tunnel at system boot using `scripts/run_tunnel.ps1`, parses the dynamic public HTTPS URL of the tunnel, and publishes it automatically to `public/backend_url.txt` in the GitHub repository.
*   **Routing proxy**: Supabase library (`@supabase/supabase-js`) is completely removed. All query and auth calls are intercepted inside `src/supabase.ts` and `Login.tsx` and dynamically routed to the active public tunnel URL fetched relatively from `/backend_url.txt` hosted on Netlify. This links the Netlify production website directly to this local PC backend 24/7.
